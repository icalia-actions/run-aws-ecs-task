...
ARG APP_PATH=/icalia-actions/run-aws-ecs-task
...
ARG APP_PATH=/icalia-actions/run-aws-ecs-task
COPY --from=builder --chown=node:node ${APP_PATH} /icalia-actions/run-aws-ecs-task
WORKDIR /icalia-actions/run-aws-ecs-task
